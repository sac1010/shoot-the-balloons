...
function App() {
  ...
  return (
    <div className="app">
      <div className="main">
        {balloons.map((el)=><Balloons key={el[1]} color={el[0]} num={el[1]}></Balloons>)}
      </div>

      <div className="empty" style={{display:"flex", gap:"10px", flexWrap:"wrap", padding:"10px"}}>
      {added.map((el)=>{
        return(
      ...
      <div className="shoot">
      <TextField  onChange={(e)=>setShootNum(e.target.value)} id="outlined-basic" label="enter a number" variant="outlined" type="number" />
          <Button onClick={handleClick} variant='contained'>shoot</Button>
      </div>
    </div>
